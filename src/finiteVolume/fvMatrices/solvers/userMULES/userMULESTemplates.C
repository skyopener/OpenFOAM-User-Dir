...
void Foam::userMULES::explicitSolve
(
    const RhoType& rho,
    volScalarField& psi,
...
{
    psi.correctBoundaryConditions();
    limit(rho, psi, epsilon, phi, phiPsi, Sp, Su, psiMax, psiMin, 3, false);
    explicitSolve(rho, psi, phiPsi, Sp, Su);
}
...
template<class RhoType, class SpType, class SuType>
void Foam::userMULES::implicitSolve
(
    const RhoType& rho,
    volScalarField& psi,
    const surfaceScalarField& phi,
...
)
{
    ...
    {
        psiMaxn[celli] = min(psiMaxn[celli], epsilon[celli]);
    }
    psiMinn = max(psiMinn, psiMin);
...
void Foam::userMULES::limitSum(SurfaceScalarFieldList& phiPsiCorrs)
{
    {
